"""Comparison collectors for gathering human and synthetic feedback."""
...
class SyntheticComparisonCollector:
    """Collects comparisons using the true reward as a synthetic oracle."""
    ...
    def add_segment_pair(self, left_seg, right_seg):
        ...
        comparison = {"left": left_seg, "right": right_seg, "label": None}
...
def _write_and_upload_video(env_id, gcs_path, local_path, segment):
    """Write a segment to video and upload to GCS."""
...
class HumanComparisonCollector:
    """Collects comparisons from human feedback via the webapp."""
    ...
    def __init__(self, env_id, experiment_name):
        ...
        if Comparison.objects.filter(experiment_name=experiment_name).count() > 0:
            raise EnvironmentError(
                "Existing experiment named %s! Pick a new experiment name." % experiment_name
            )

    def convert_segment_to_media_url(self, comparison_uuid, side, segment):
        """Convert a segment to a media URL by uploading to GCS."""
    ...
    def label_unlabeled_comparisons(self):
        """Check the webapp for new labels and apply them."""
        from human_feedback_api import Comparison
